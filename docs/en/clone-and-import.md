...
`git clone https://github.com/ByteLegendQuest/java-deep-copy`

A project directory named `java-deep-copy` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-deep-copy` directory which is created just now.
